...
import { Menu, X, ChevronRight, Phone, Diamond, Mail } from "lucide-react";
        ...
        <nav className="container-luxury flex h-20 items-center justify-between">
          ...
          <Link href="/" className="group flex items-center gap-3.5" aria-label={SITE_CONFIG.name}>
            ...
            <div className="flex flex-col leading-none">
              ...
              <span
                className={cn(
                  "text-[9px] uppercase tracking-[0.35em] font-semibold transition-colors duration-500",
                  scrolled ? "text-gold-600" : "text-gold-champagne"
                )}
              >
                Properties Limited
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  className={cn(
                    "relative px-[18px] py-2.5 text-[13px] font-semibold uppercase tracking-[0.12em] transition-all duration-300",
                    scrolled
                      ? active
                        ? "text-gold-600"
